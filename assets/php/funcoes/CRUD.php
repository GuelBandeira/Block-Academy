...
class CRUD {
    ...
    */
    ...
    public static function SELECT_ID_W($campos, $tabela, $id){
        ...
        public static function SELECT_ID_W_A($campos, $tabela, $id){
        
        $DB = Conexao::getInstance();
        $a = $DB->query("SET NAMES utf8;SET character_set_connection=utf8;SET character_set_client=utf8;SET character_set_results=utf8;SET time_zone='-3:00';");
        $a->closeCursor();

        $query = "SELECT ".($campos!=''?$campos:"*")." FROM $tabela WHERE id_users=:id AND moeda=:moeda";
        $sql = $DB->prepare($query);

        $sql->bindValue(':id', $id, PDO::PARAM_INT);
        $sql->bindValue(':moeda', 'AGROCASH');
